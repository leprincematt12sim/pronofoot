...
function toggleShuffle() {
  ...
  if (btn) btn.style.color = isShuffle ? 'var(--accent)' : 'var(--text-muted)';
}
...
function compressAudioFile(file, callback) {
  if (file.size > 10 * 1024 * 1024) { alert("Le fichier est trop grand (max: 10 Mo)."); return; }
...
function adminAddMusic(event) {
  ...
  compressAudioFile(file, async function(dataUrl, name) {
    ...
    setupAudioPlayer();
    renderAdminPlaylist();
    alert(`🎵 Morceau "${newSong.name}" ajouté à la playlist !`);
  });
}
...
// CONNEXION REPARÉE & SÉCURISÉE (LOCAL + CLOUD)
async function handleLogin(e) {
  ...
  // 2. Chercher sur Firestore si disponible
  if (firestore) {
    try {
...
function renderDashboardLeaderboard() {
  ...
  sorted.slice(0, 5).forEach((u, i) => {
    ...
    html += `
      <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:${isMe?'rgba(0,230,118,0.15)':'var(--bg-card)'};border:1px solid ${isMe?'var(--accent)':'var(--border)'};border-radius:8px;margin-bottom:6px">
      ...
      if (pred && pred.h !== '' && pred.a !== '') {
        ...
        html += `
          <div style="display:flex;align-items:center;gap:8px;padding:4px 0;font-size:0.85rem">
            ...
            <span style="background:var(--bg-body);padding:2px 8px;border-radius:4px;font-weight:bold">${pred.h} - ${pred.a}</span>
            ${resultLabel}
          </div>`;
      }
    });
  ...
  c.innerHTML = BONUS_CONFIG.map(b => {
    ...
    return `
      <div class="admin-card">
        ...
        <select id="bonus_${b.id}" style="width:100%;margin-top:8px;padding:8px;background:var(--bg-body);border:1px solid var(--border);color:white;border-radius:6px">
  ...
  ALL_MATCHES.slice(0, 50).forEach(m => {
    ...
    html += `
      <div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid var(--border);flex-wrap:wrap">
        ...
        <input type="number" min="0" max="15" value="${s?s.h:''}" id="ah_${m.id}" placeholder="H" style="width:38px;padding:5px;text-align:center;background:var(--bg-body);border:1px solid var(--border);color:white;border-radius:4px;font-weight:bold">
        <span>-</span>
        <input type="number" min="0" max="15" value="${s?s.a:''}" id="aa_${m.id}" placeholder="A" style="width:38px;padding:5px;text-align:center;background:var(--bg-body);border:1px solid var(--border);color:white;border-radius:4px;font-weight:bold">
  ...
  for (const [key, li] of Object.entries(LEAGUE_INFO)) {
    ...
    html += `
      <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border);flex-wrap:wrap">
        ...
        <label style="background:var(--accent);color:#000;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:0.8rem;font-weight:bold">
          📁 Fond PC
...
function navigateTo(p) {
  if (p === 'admin') {
    if (!currentUser || currentUser.role !== 'admin') {
      alert("⛔ Accès refusé ! Cette section est strictement réservée à l'administrateur.");
      return;
    }
  }
  ...
  if (p === 'admin') { renderAdminMatchList(); renderAdminStats(); renderAdminLeagueBanners(); renderAdminLeagueBackgrounds(); renderAdminPlaylist(); }
