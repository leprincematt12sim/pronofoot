...
let appState = {
  ...
  settings: {
    ...
    themeColor: '#e94560',
    apiKey: REAL_API_KEY,
    bgMusic: localStorage.getItem('pf_bg_music') || null,
    bgMusicName: localStorage.getItem('pf_bg_music_name') || 'Musique PronoFoot'
  }
};
...
// TÉLÉCHARGEMENT DU FICHIER MP3 PAR L'ADMINISTRATEUR
function uploadAudioFromFile(event) {
  const file = event.target.files[0];
  if (!file) return;

  if (file.size > 8 * 1024 * 1024) {
    alert("⚠️ Le fichier MP3 est trop volumineux (plus de 8 Mo). Choisissez un morceau plus court.");
    return;
  }
  ...
  reader.onload = async function(e) {
    ...
    appState.settings.bgMusic = audioData;
    appState.settings.bgMusicName = file.name;

    try {
      localStorage.setItem('pf_bg_music', audioData);
      localStorage.setItem('pf_bg_music_name', file.name);
    } catch(err) {}
    ...
    setupAudioPlayer();
    document.getElementById('audioFileNameDisplay').textContent = "✅ Musique chargée : " + file.name;
    alert(`🎵 Musique "${file.name}" chargée et diffusée sur le site !`);
  };
  reader.readAsDataURL(file);
}

function setupAudioPlayer() {
  const player = document.getElementById('siteAudioPlayer');
  const titleDisplay = document.getElementById('musicTitleDisplay');

  if (appState.settings.bgMusic) {
    player.src = appState.settings.bgMusic;
    titleDisplay.textContent = appState.settings.bgMusicName || "Ambiance Foot";
  } else {
    // Musique d'ambiance football par défaut
    player.src = "https://cdn.pixabay.com/download/audio/2022/05/27/audio_1808fbf07a.mp3?filename=stadium-ambient-112323.mp3";
    titleDisplay.textContent = "Ambiance Stade";
  }
}
